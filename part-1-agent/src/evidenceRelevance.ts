...
export function scoreEvidence(
  options: AgentOptions,
  source: TrendEvidence,
): TrendEvidence {
  const nicheTerms = relevantTerms(
    `${options.profile.niche} ${options.profile.profile}`,
  );
  const location = options.profile.location.split(",")[0].trim().toLowerCase();
  const metadataOnlySocial =
    source.scrapeStatus === "metadata_only" &&
    ["tiktok", "youtube", "reddit", "x"].includes(source.platform);
  ...
  const hasTrendSignal =
    /trend|format|hook|short-form|tiktok|viral|creator|sound|hashtag/i.test(
      text,
    );
  const tier =
    directScore >= 2
      ? "direct"
      : isSupportingSurface && hasTrendSignal
        ? "supporting"
        : "weak";
  ...
  return {
    ...
    relevanceTier: tier,
    relevanceScore:
      directScore + (isSupportingSurface && hasTrendSignal ? 1 : 0),
    qualityNotes: [
      ...(source.qualityNotes ?? []),
