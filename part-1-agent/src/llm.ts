...
export async function synthesizeTrendVideoReport(
  ...
  const synthesis = await synthesizeReportShapeWithFallback(
    options,
    evidence,
    candidates,
    failureNotes,
  ...
  const parsed = completeReportShape(
    repairReportShape(synthesis.report),
    candidates,
    options,
  );
  const futureVideoPipelineDraft = buildFuturePipelineDraft(
    options,
    parsed.recommendedConcept,
  );
  ...
  return {
    ...
    failureNotes: [...failureNotes, ...synthesis.failureNotes],
    tradeoff:
...
async function synthesizeReportShapeWithFallback(
  ...
  candidates: TrendCandidate[],
  failureNotes: string[],
): Promise<{ report: ReportShape; failureNotes: string[] }> {
  try {
    return {
      report: await synthesizeReportShape(
        options,
        evidence,
        candidates,
        failureNotes,
    ...
    console.warn(
      `LLM synthesis failed, using deterministic fallback: ${message}`,
    );
    return {
      report: fallbackReportShape(options, candidates),
      failureNotes: [
        `LLM synthesis failed, so the report used deterministic profile-specific synthesis: ${message}`,
      ],
    };
  }
}
...
async function synthesizeReportShape(
  ...
  if (anthropicKey)
    return synthesizeWithAnthropic(
      options,
      evidence,
      candidates,
      failureNotes,
  ...
  throw new Error(
    "Missing LLM key. Set ANTHROPIC_API_KEY or OPENAI_API_KEY in .env.",
  );
}
...
async function synthesizeWithAnthropic(
  ...
  if (!response.ok)
    throw new Error(
      `Anthropic request failed: ${payload.error?.message ?? response.statusText}`,
    );
  if (payload.stop_reason === "max_tokens") {
    throw new Error(
      `Anthropic reached its ${maxTokens}-token output limit before completing the report.`,
    );
  }
...
function completeReportShape(
  ...
  const seen = new Set(
    report.rankedTrends.map((trend) => trend.name.toLowerCase()),
  );
  const filledTrends: TrendCandidate[] = report.rankedTrends.map((trend) => {
    ...
    return {
      ...
      validationLevel:
        trend.validationLevel ?? source?.validationLevel ?? "weak",
      sourceDiversity:
        (trend.sourceDiversity ??
          source?.sourceDiversity ??
          new Set(trend.exampleUrls.map((url) => urlDomain(url))).size) ||
        1,
      platformDiversity:
        trend.platformDiversity ?? source?.platformDiversity ?? 1,
      independentSourceCount:
        trend.independentSourceCount ??
        source?.independentSourceCount ??
        trend.sourceDiversity ??
        1,
...
function normalizeTrend(trend: TrendCandidate): TrendCandidate {
  return {
    ...
    independentSourceCount:
      trend.independentSourceCount ?? trend.sourceDiversity ?? 1,
  };
}
...
function buildConceptFromCandidate(
  options: AgentOptions,
  candidate: TrendCandidate,
): CreativeConcept {
  const humanShot =
    candidate.productionFit === "high" ||
    candidate.platform !== "cross_platform";
  const productionMode = humanShot
    ? candidate.platform === "creative_center"
      ? "hybrid"
      : "human_shot"
    : "hybrid";
  ...
  return {
    ...
    executionStyle:
      "Fast, credible vertical video using real proof, readable text overlays, and a human or product-led demonstration.",
    productionMode,
    scenePlan,
    shotList: [
...
function buildFuturePipelineDraft(
  options: AgentOptions,
  ...
  return {
    ...
    bgmMood:
      concept.productionMode === "human_shot"
        ? "warm upbeat lo-fi"
        : "soft trending pop with cozy pacing",
...
function fallbackCandidate(options: AgentOptions): TrendCandidate {
  return {
    ...
    topic:
      "show the audience problem, credible intervention, and practical result",
    score: 12,
...
function repairReportShape(report: ReportShape): ReportShape {
  return {
    ...
    recommendedConcept: {
      ...
      aiVideoPrompt: report.recommendedConcept.aiVideoPrompt
        ? repairText(report.recommendedConcept.aiVideoPrompt)
        : undefined,
    },
  ...
  properties: {
    ...
    recommendedConcept: {
      ...
      properties: {
        ...
        productionMode: {
          type: "string",
          enum: ["human_shot", "ai_generated", "hybrid"],
        },
