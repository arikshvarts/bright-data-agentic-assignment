...
describe("runTrendVideoAgent", () => {
  it("produces a report without live API calls", async () => {
    const client: ToolClient = {
      async callTool<T = unknown>(call: ToolCall): Promise<T> {
        ...
        if (call.name === "scrape_as_markdown") {
          return "2026 TikTok trend evidence with POV, cozy cafe, coffee ASMR, local Tel Aviv hidden gem, remote work, students, and example videos. ".repeat(8) as T;
        }
    ...
    expect(report.toolsUsed).toEqual(["search_engine", "discover", "scrape_as_markdown"]);
    expect(report.rankedTrends.length).toBeGreaterThanOrEqual(3);
    expect(report.recommendedConcept.scenePlan.length).toBeGreaterThanOrEqual(4);
